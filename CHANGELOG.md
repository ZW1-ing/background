...
- 第二次及后续点击“应用背景”会继续传递副本专用放行参数，不再重复触发
  Electron 标记校验错误。
- Windows 应用检测按真实路径去重，避免同一安装重复出现在选择列表。
- 补丁子进程统一使用 UTF-8 输出，兼容非中文代码页的 Windows 环境。
...
- Windows 优先选择 `%LOCALAPPDATA%\Programs` 等可修改的普通桌面安装。
- 保存的 Store / WindowsApps 目标不可修改时，如果存在普通桌面版，会自动改用普通版。
- 只有 Store 版时，控制面板会标记“不支持”并禁用应用操作。
- 应用失败时保留补丁脚本的完整输出，方便定位权限、路径和完整性校验问题。
- 修复前端 API 脚本的加载路由。
